...
public class TimeUtil {
     ...
     * yyyy.MM.dd-HH:mm
     */
    public final static DateTimeFormatter YYYYMMDDGHHMM = DateTimeFormatter.ofPattern("yyyy.MM.dd-HH:mm")
            .withLocale(Locale.getDefault());
    /**
     * yyyy-MM-dd HH:mm
     */
    ...
    public static long getOffToDayZeroMil(int days) {
        try {
            ZonedDateTime ldt = ZonedDateTime.ofInstant(Instant.ofEpochMilli(currentTimeMillis()),
            ...
            ldt = ZonedDateTime.of(ldt.getYear(), ldt.getMonthValue(), ldt.getDayOfMonth(), 0, 0, 0, 0,
                    ZoneId.systemDefault());
    ...
    /**
     * 获取与今日相差天数的日期23.59.59的毫秒数，负为日期前，正为日期后。如yyyy-MM-dd HH
     *
     * @param days
     * @return
     */
    public static long getOffToDayMaxZeroMil(int days) {
        try {
            ZonedDateTime ldt = ZonedDateTime.ofInstant(Instant.ofEpochMilli(currentTimeMillis()),
                    ZoneId.systemDefault());
            if (days < 0) {
            ...
            ldt = ZonedDateTime.of(ldt.getYear(), ldt.getMonthValue(), ldt.getDayOfMonth(), 23, 59, 59, 0,
                    ZoneId.systemDefault());
            return ldt.toInstant().toEpochMilli();
        } catch (Exception e) {
            LOGGER.error("", e);
    ...
    public static int getMinDaysOfMonth() {
        return LocalDateTime.ofInstant(Instant.ofEpochMilli(currentTimeMillis()), ZoneId.systemDefault()).getMonth()
                .minLength();
    }
    ...
    public static int getDayOfWeek() {
        return LocalDateTime.ofInstant(Instant.ofEpochMilli(currentTimeMillis()), ZoneId.systemDefault()).getDayOfWeek()
                .getValue();
    }
    ...
    public static boolean isSameDay(long time1, long time2) {
        return getDateTimeFormat(time1, YYYYMMDD).equals(getDateTimeFormat(time2, YYYYMMDD));
    }
    ...
    public static boolean isSameYear(long time1, long time2) {
        return LocalDateTime.ofInstant(Instant.ofEpochMilli(time1), ZoneId.systemDefault()).getYear() == LocalDateTime
                .ofInstant(Instant.ofEpochMilli(time2), ZoneId.systemDefault()).getYear();
    }
     ...
     * @return
     */
    public static boolean isSameMinu(long time1, long time2) {
        return LocalDateTime.ofInstant(Instant.ofEpochMilli(time1), ZoneId.systemDefault()).getMinute() == LocalDateTime
                .ofInstant(Instant.ofEpochMilli(time2), ZoneId.systemDefault()).getMinute();
    }
     ...
     * @return
     */
    public static long minuteOffset(long time1, long time2) {
        return Duration.ofMillis(time1).toHours() - Duration.ofMillis(time2).toHours();
    }
     ...
     * @return
     */
    public static long hourOffset(long time1, long time2) {
        return Duration.ofMillis(time1).toHours() - Duration.ofMillis(time2).toHours();
    }
    ...
    public static long currentTimeMillis() {
        // Clock 底层也是使用 System.currentTimeMillis()
        // return Clock.systemDefaultZone().instant().toEpochMilli() + timeOffset;
        return System.currentTimeMillis() + timeOffset;
    }

    /**
     * @return 获取当前纪元秒 1970-01-01T00:00:00
     */
    public static int currentTimeSecond() {
        return (int) (currentTimeMillis() / 1000);
    }

    public static long currentOffsetTimeMillis(int offsetDays, int hour, int minute, int secord) {
...
//		return Clock.systemDefaultZone().instant().getEpochSecond() + timeOffset / 1000;
        return System.currentTimeMillis() / 1000 + timeOffset / 1000;
    }
    ...
    public static boolean setCurrentDateTime(String datetime) {
        ...
        LOGGER.info("修改服务器时间成功：======" + timeOffset);
        return true;
    }
    ...
    public static final ZoneOffset ZONE_OFFSET = ZoneOffset
            .ofTotalSeconds(Calendar.getInstance().getTimeZone().getRawOffset() / 1000);
    ...
    public static long getTimeByLocal(int year, int month, int day, int hour, int minte, int second) {
        return LocalDateTime.of(year, month, day, hour, minte, second).atZone(ZONE_OFFSET).toInstant().toEpochMilli();
    }
    ...
    /**
     * 判断两个时间是否在同一周(注意这里周日和周一判断是在一周里的) <br>
     * 星期一为第一天
     *
     * @param time1
     * @param time2
     * @return
     */
    public static boolean isSameWeekISO(long time1, long time2) {
        LocalDateTime ldt1 = LocalDateTime.ofInstant(Instant.ofEpochMilli(time1), ZoneId.systemDefault());
        LocalDateTime ldt2 = LocalDateTime.ofInstant(Instant.ofEpochMilli(time2), ZoneId.systemDefault());
        TemporalField woy = WeekFields.ISO.weekOfWeekBasedYear();
        if (ldt1.get(woy) == ldt2.get(woy)) {
            if (ldt1.getYear() == ldt2.getYear()) {
                return true;
                //夸年
            } else if (ldt1.get(woy) == 52 && Math.abs(ldt1.getYear() - ldt2.getYear()) < 2) {
                return true;
            }
        }
     ...
     * @return
     */
    public static long getWeekOfDayBegin(DayOfWeek day) {
        LocalDateTime nowMonday = LocalDateTime.of(LocalDate.now(), LocalTime.MIN).with(day).withHour(0).withMinute(0)
                .withSecond(0);
        return nowMonday.toInstant(ZONE_OFFSET).toEpochMilli();
    }
     ...
     * @return
     */
    public static long getWeekOfDayEnd(DayOfWeek day) {
        LocalDateTime nowMonday = LocalDateTime.of(LocalDate.now(), LocalTime.MIN).with(day).withHour(23).withMinute(59)
                .withSecond(59);
        return nowMonday.toInstant(ZONE_OFFSET).toEpochMilli();
    }
