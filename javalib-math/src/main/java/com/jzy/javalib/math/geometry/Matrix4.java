package com.jzy.javalib.math.geometry;
...
public class Matrix4 implements Serializable {
	 ...
	 * 
	 * @param values
	 *            The matrix, in float form, that is to be copied. Remember that
	 *            this matrix is in
	 *            <a href="http://en.wikipedia.org/wiki/Row-major_order">column
	 *            major</a> order.
	 * @return This matrix for the purpose of chaining methods together.
	 ...
	 * 
	 * @param quaternionX
	 *            The X component of the quaternion that is to be used to set this
	 *            matrix.
	 * @param quaternionY
	 *            The Y component of the quaternion that is to be used to set this
	 *            matrix.
	 * @param quaternionZ
	 *            The Z component of the quaternion that is to be used to set this
	 *            matrix.
	 * @param quaternionW
	 *            The W component of the quaternion that is to be used to set this
	 *            matrix.
	 * @return This matrix for the purpose of chaining methods together.
	 */
	 ...
	 * 
	 * @param translationX
	 *            The X component of the translation that is to be used to set this
	 *            matrix.
	 * @param translationY
	 *            The Y component of the translation that is to be used to set this
	 *            matrix.
	 * @param translationZ
	 *            The Z component of the translation that is to be used to set this
	 *            matrix.
	 * @param quaternionX
	 *            The X component of the quaternion that is to be used to set this
	 *            matrix.
	 ...
	 * 
	 * @param vector
	 *            The translation vector to add to the current matrix. (This vector
	 *            is not modified)
	 * @return This matrix for the purpose of chaining methods together.
	 */
	 ...
	 * @param normalizeAxes
	 *            True to normalize the axes, necessary when the matrix might also
	 *            include scaling.
	...
	public float getScaleX() {
		return (MathUtil.isZero(val[Matrix4.M01]) && MathUtil.isZero(val[Matrix4.M02])) ? Math.abs(val[Matrix4.M00])
				: (float) Math.sqrt(getScaleXSquared());
	}

	/** @return the scale factor on the Y axis (non-negative) */
	public float getScaleY() {
		return (MathUtil.isZero(val[Matrix4.M10]) && MathUtil.isZero(val[Matrix4.M12])) ? Math.abs(val[Matrix4.M11])
				: (float) Math.sqrt(getScaleYSquared());
	}

	/** @return the scale factor on the X axis (non-negative) */
	public float getScaleZ() {
		return (MathUtil.isZero(val[Matrix4.M20]) && MathUtil.isZero(val[Matrix4.M21])) ? Math.abs(val[Matrix4.M22])
				: (float) Math.sqrt(getScaleZSquared());
	}

	/**
	 * @param scale
	 *            The vector which will receive the (non-negative) scale components
	 *            on each axis.
	 ...
	 * stored in the vector array. This is the same as .
	 * 
	 * @param mat
	 *            the matrix
	 * @param vec
	 ...
	 * vectors and must be >= 3. This is the same as  Vector3*(Matrix4)
	 ...
	 * same as  Vector3#prj(Matrix4) applied to multiple vectors.
	 * 
	 * @param mat
	 *            the matrix
	 * @param vecs
	 ...
	 * vectors stored in the vectors array. The stride parameter specifies the
	 * number of floats between subsequent vectors and must be >= 3. This is the
	 * same as Vector3#rot(Matrix4) applied to multiple vectors.
	 * 
	 * @param mat
	 *            the matrix
	 * @param vecs
	...
	public boolean hasRotationOrScaling() {
		return !(MathUtil.isEqual(val[M00], 1) && MathUtil.isEqual(val[M11], 1) && MathUtil.isEqual(val[M22], 1)
				&& MathUtil.isZero(val[M01]) && MathUtil.isZero(val[M02]) && MathUtil.isZero(val[M10])
				&& MathUtil.isZero(val[M12]) && MathUtil.isZero(val[M20]) && MathUtil.isZero(val[M21]));
	}
}
